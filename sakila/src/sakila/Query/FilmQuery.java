...
public class FilmQuery {
	...
	public static final String SELECT_FILM_LIST_COUNT
	= "select count(*) from film_list fl inner join film  f on fl.FID = f.film_id inner join language  l on f.language_id = l.language_id";
	public static final String SELECT_FILM_INVENTORY_COUNT
	= "SELECT COUNT(*) FROM (SELECT COUNT(i.film_id) FROM  film_list fl inner join film f on fl.FID = f.film_id inner join language l on f.language_id = l.language_id inner join inventory i on f.film_id = i.film_id group by i.film_id) listCount";
}
